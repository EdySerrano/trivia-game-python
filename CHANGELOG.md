# CHANGELOG.md


## Dia 1 - 2025-04-09
- Comfiguracion del proyecto, creacion de la carpeta trivia-game-python
- Instalacion de FastAPI, Uvicorn y otras dependencias
- Creacion del `Docker file`
- Configuracion del `Docker-compose.yml`
- Inicializaciony creacion de la rama: `develop`
- Commit: `Configuración inicial del proyecto y archivos Docker`





## Dia 2 – 2025-04-10
- Implementación de la clase `Question` en `trivia.py`
- Pruebas unitarias en `test_trivia.py`
- Se validó con pytest
- Commit: "Dia2: Implementación de la clase Question y pruebas unitarias básicas"
- Rama: `feature/dia2`
- Tag: `v1.0-day2`



## Dia 3 - 2025-04-11
- Implementar la clase `Quiz` en `Trivia.py`.
- Agregar la logica de interaccion basica en una funcion.
- commit: "Implementación de la clase Quiz y flujo básico del juego"
- Rama: `feature/dia3` and `feature/estructura-basic`
- tag: `v1.0-day3`

## Dia 4 - 2025-04-12
- Modificar la clase `Quiz`
- Actualizar pruebas unitarias `test_quiz_scoring()`
- Implementar la funcion `run_Quiz()`
- Commit : "Implementación de sistema de puntuación, manejo de rondas y finalización del juego"
- Rama: `feature/dia4`
- tag: `v1.0-day4`


## Dia 5 - 2025-04-13
- Mejorar la clase `run_quiz()`
- Actualizar pruebas unitarias `test_quiz_multiple_question()`
- Implementar la función `run_quiz()` para el flujo de 10 rondas.
- Commit :"Implementación de sistema de puntuación, manejo de rondas y finalización del juego"
- Rama: `feature/dia5`
- tag: `v1.0-day5`


## Dia 6 - 2025-04-14
- Crear una rama para CI/CD
- Crear el archivo de workflow en gitHubActions
- Configurar `SonarQube`
- Implementar las pruebas de integracion para la API
- Commit:"Configuración de pipeline CI/CD y pruebas de integración"
- rama: `feature/dia6`
- tag: `v1.0-day6`

